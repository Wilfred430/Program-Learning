...
int main()
{
    ...
    string startext, nulltext, modeltext;
    ...
    while (1)
    {
        ...
        while (!text.eof())
        {
            getline(text, modeltext); // 以列為單位，印出遊戲畫面
            cout << "     " << modeltext << endl;
        }
        Sleep(600);
        text.close();  // 關閉檔案
...
void player1turn(char arrow[7], char dot[][7]) // 玩家1的回合
{
    ...
    while (c != 32) // 迴圈，直到按下空白鍵 (Space ascii code為32)
    {
        ...
        else
        {
            ...
            if (num == 42)                         // 棋盤滿
                tie();                             // 呼叫平手畫面並結束遊戲
            dot_set(arrow, dot, player1, player2); // 下子並判斷是否連線並用bool判斷是player1的回合
            refresh(arrow);
            gotoxy(0, 0);
            num++; // 記錄下幾個子
    ...
    if (num != 42)             // 非滿格
        fullcheck(dot, arrow); // 確認是否滿行 fullcheck會回傳changeplayer布林值來決定是否交換player
    dot_set(arrow, dot, player1, player2);
    refresh(arrow);
    gotoxy(0, 0);
    passtime = time(NULL) - start + Gap;
...
void player2turn(char arrow[7], char dot[][7])
{
    ...
    else if (robot == true)
    {
        ...
        full_line2.clear();
        srand(time(NULL));
        Sleep((rand() % 1501) + 500); // 電腦隨機下棋時間 1.5 ~ 2 sec
        refresh(arrow);
        system("cls");
        arrow[k] = 'V';
        cout << endl
    ...
    dot_set(arrow, dot, player1, player2); // 下子並判斷player2是否勝出用bool判斷是否為player2的回合
    refresh(arrow);
    gotoxy(0, 0);
    passtime = time(NULL) - start + Gapk;
    if (changeplayer || passtime <= 0)
    ...
    if (count >= 4)
    {
        ...
        for (i = 1; i < 7; i++)
        {
            if (x + i <= 6 && y - i >= 0)
            {
                ...
                {
                    dot[y - i][x + i] = 'A'; // 存A以方便在之後四子連線時轉為紅色
                    break;
                }
                else if (dot[y][x] == dot[y - i][x + i])
                {
...
void Gamestartanimation()
{
    ...
    ifstream text;
    string startext;
    text.open("Loading.txt"); // 載入遊戲讀取貓咪頁面
    while (count < 20)
    {
        getline(text, startext); // 以列為單位，印出遊戲畫面
        cout << "      " << startext << endl;
...
void fullline(char dot[][7], char arrow[7]) // 給自動下棋vector那邊用的函式，因為跟fullcheck架構不太一樣不能共用，才再寫一個
{
    int count = 0;
    for (int i = 0; i < 7; i++)
    {
